...
func TestCreateUser(t *testing.T) {
	...
	resp, err := http.Post(ts.URL+"/users", "application/json",
		strings.NewReader(`{"first_name":"hyunsang", "last_name":"park", "email":"parkhyunsang@kakao.com"}`))
	assert.NoError(err)
...
func TestUpdateUser(t *testing.T) {
	...
	req, _ := http.NewRequest("PUT", ts.URL+"/users",
		strings.NewReader(`{"id":1, "first_name":"updated", "last_name":"updated", "email":"updated@kakao.com"}`))
	resp, err := http.DefaultClient.Do(req)
	assert.NoError(err)
	assert.Equal(http.StatusOK, resp.StatusCode)
	data, _ := ioutil.ReadAll(resp.Body)
	...
	updateStr := fmt.Sprintf(`{"id":%d, "first_name":"hyunsang"}`, user.ID)
	...
	assert.Equal("hyunsang", updateUser.FirstName)
...
func TestUsers_withUsersData(t *testing.T) {
	...
	resp, err := http.Post(ts.URL+"/users", "application/json",
		strings.NewReader(`{"first_name":"aaron", "last_name":"park", "email":"aaron@kakao.com"}`))
	assert.NoError(err)
	assert.Equal(http.StatusCreated, resp.StatusCode)
